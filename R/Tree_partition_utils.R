...
treenode2cluster <- function(cluster_results = NULL,
  ...
  cluster_results$agglomeration.stats$Motifs <- sapply(leaves.per.node(tree, labels = TRUE), function(cl){ paste(sort(cl), collapse = ",")})
  ...
  nb.char.cluster.ids     <- nchar(as.character(length(clusters))) ## Number of digits
  sprint.string           <- paste0("%0", nb.char.cluster.ids, "d")     ## Sprintf options  
  node2cluster.df$cluster <- paste0("cluster_", sprintf(sprint.string, node2cluster.df$cluster))
  cluster.zero <- paste0("cluster_", paste0(rep(0, each = nb.char.cluster.ids), collapse = ""))
  node2cluster.df$cluster <- gsub(pattern = cluster.zero, replacement = 0, node2cluster.df$cluster)
  
  return(node2cluster.df)
}
...
treeleaf2cluster <- function(node2cluster_tab = NULL,
  ...
  nb.char.cluster.ids  <- nchar(as.character(max(leaf.to.cluster.table$cluster))) ## Number of digits
  sprint.string        <- paste0("%0", nb.char.cluster.ids, "d")     ## Sprintf options  
